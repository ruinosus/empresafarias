    ...
    class RepositorioHistoricoTitular : IRepositorioHistoricoTitular
    {
        ...
        public HistoricoTitular Inserir(HistoricoTitular historicoTitular)
        {
            ...
            try
            {
                ...
                comando.Parameters.AddWithValue("@Nome", historicoTitular.Nome);
                comando.Parameters.AddWithValue("@CidadeNaturalidadeId", historicoTitular.CidadeNaturalidade.Id);
                comando.Parameters.AddWithValue("@Cpf", historicoTitular.Cpf);
                comando.Parameters.AddWithValue("@DataNascimento", historicoTitular.DataNascimento);
                comando.Parameters.AddWithValue("@Bairro", historicoTitular.Endereco.Bairro);
                comando.Parameters.AddWithValue("@Cep", historicoTitular.Endereco.Cep);
                comando.Parameters.AddWithValue("@CidadeEnderecoId", historicoTitular.Endereco.Cidade.Id);
                comando.Parameters.AddWithValue("@Complemento", historicoTitular.Endereco.Complemento);
                comando.Parameters.AddWithValue("@Logradouro", historicoTitular.Endereco.Logradouro);
                comando.Parameters.AddWithValue("@Numero", historicoTitular.Endereco.Numero);
                comando.Parameters.AddWithValue("@EstadoCivil", historicoTitular.EstadoCivil);
                comando.Parameters.AddWithValue("@Rg", historicoTitular.Rg.Numero);
                comando.Parameters.AddWithValue("@DataExpedicao", historicoTitular.Rg.DataExpedicao);
                comando.Parameters.AddWithValue("@OrgaoExpeditor", historicoTitular.Rg.OrgaoExpeditor);
                comando.Parameters.AddWithValue("@Sexo", historicoTitular.Sexo);
                comando.Parameters.AddWithValue("@Status", (int)historicoTitular.Status);
                comando.Parameters.AddWithValue("@TelefoneCelular", historicoTitular.TelefoneCelular);
                comando.Parameters.AddWithValue("@TelefoneResidencial", historicoTitular.TelefoneResidencial);
                ...
                historicoTitular.Id = this.ObterMaximoId();
            }
        ...
        private HistoricoTitular CriarHistoricoTitular(SqlDataReader resultado)
        {
            ...
            if (resultado["Id"] != DBNull.Value)
            {
                historicoTitular.Id = Convert.ToInt32(resultado["Id"]);
            }
            if (resultado["DataNascimento"] != DBNull.Value)
            {
                historicoTitular.DataNascimento = Convert.ToDateTime(resultado["DataNascimento"]);
            }
            if (resultado["Nome"] != DBNull.Value)
            {
                historicoTitular.Nome = Convert.ToString(resultado["Nome"]);
            }
            if (resultado["EstadoCivil"] != DBNull.Value)
            {
                historicoTitular.EstadoCivil = Convert.ToString(resultado["EstadoCivil"]);
            }
            if (resultado["Status"] != DBNull.Value)
            {
                historicoTitular.Status = (StatusTitular)Convert.ToInt32(resultado["Status"]);
            }
            if (resultado["Cpf"] != DBNull.Value)
            {
                historicoTitular.Cpf = Convert.ToString(resultado["Cpf"]);
            }
            if (resultado["CidadeNaturalidadeId"] != DBNull.Value)
            {
                historicoTitular.CidadeNaturalidade.Id = Convert.ToInt32(resultado["CidadeNaturalidadeId"]);
            }
            if (resultado["Rg"] != DBNull.Value)
            {
                historicoTitular.Rg.Numero = Convert.ToString(resultado["Rg"]);
            }
            if (resultado["DataExpedicao"] != DBNull.Value)
            {
                historicoTitular.Rg.DataExpedicao = Convert.ToDateTime(resultado["DataExpedicao"]);
            }
            if (resultado["OrgaoExpeditor"] != DBNull.Value)
            {
                historicoTitular.Rg.OrgaoExpeditor = Convert.ToString(resultado["OrgaoExpeditor"]);
            }
            if (resultado["Sexo"] != DBNull.Value)
            {
                historicoTitular.Sexo = Convert.ToChar(resultado["Sexo"]);
            }
            if (resultado["TelefoneCelular"] != DBNull.Value)
            {
                historicoTitular.TelefoneCelular = Convert.ToString(resultado["TelefoneCelular"]);
            }
            if (resultado["TelefoneResidencial"] != DBNull.Value)
            {
                historicoTitular.TelefoneResidencial = Convert.ToString(resultado["TelefoneResidencial"]);
            }
            if (resultado["Logradouro"] != DBNull.Value)
            {
                historicoTitular.Endereco.Logradouro = Convert.ToString(resultado["Logradouro"]);
            }
            if (resultado["Bairro"] != DBNull.Value)
            {
                historicoTitular.Endereco.Bairro = Convert.ToString(resultado["Bairro"]);
            }
            if (resultado["Cep"] != DBNull.Value)
            {
                historicoTitular.Endereco.Cep = Convert.ToString(resultado["Cep"]);
            }
            if (resultado["CidadeEnderecoId"] != DBNull.Value)
            {
                historicoTitular.Endereco.Cidade.Id = Convert.ToInt32(resultado["CidadeEnderecoId"]);
            }
            if (resultado["Complemento"] != DBNull.Value)
            {
                historicoTitular.Endereco.Complemento = Convert.ToString(resultado["Complemento"]);
            }
            if (resultado["Numero"] != DBNull.Value)
            {
                historicoTitular.Endereco.Numero = Convert.ToString(resultado["Numero"]);
            }
